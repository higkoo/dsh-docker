...
```bash
# 方式一：临时覆盖（推荐）
docker run -d ... -e TZ=Asia/Tokyo ghcr.io/higkoo/dsh:v0.3.9
...
|------|------|----------|
| `latest` | 最新稳定版 | 日常使用 |
| `v0.3.9` | 语义化版本，固定不变 | **生产环境推荐**，避免意外升级 |
...
**版本线说明**：`v0.1.*` 系列已停止维护并从镜像仓库移除，请使用 `v0.3.9` 及以上版本。
...
[4/6] 启动 DSH Web UI（首次启动需初始化插件，请耐心等待）...
  ...
  [dsh] 正在加载插件 dsh-ctl ...
  [dsh] 正在加载插件 dsh-data-analysis（首次需建 venv、装 marivo/pandas）...
  DSH 启动中... 5s（插件初始化中，超过 120s 后转为低频提示）
  DSH 启动中... 15s（插件初始化中，超过 120s 后转为低频提示）
  DSH 已就绪（18s，插件树加载完成）
  LAN 插件已生效: http://172.24.0.24:3080/
============================================================
  [5/6] DSH 已就绪，请访问:
    HTTP : http://<Your-IP>:9080/?token=<token>
    ...
```
...
```bash
git tag -a v0.3.9 -m "v0.3.9: 变更说明"
git push origin v0.3.9
```

推送后 CI 自动 lint → 构建 → 推送镜像，产出 `v0.3.9`、`v0.3`、`latest` 与 `sha-*` 标签。
