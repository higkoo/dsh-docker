...
不是访问者。DSH 放在端口映射 / 网关 / LB 后面时，这一列会变成网关或容器
地址 —— 真机反馈里看到的正是这种情况，排查问题时看不出请求来自谁。
...
这一点必须说清楚，否则容易误以为「改完配置就一定能看到真实 IP」。

`realip` 模块只能**解析**已存在的 `X-Forwarded-For`，**自己不会推断来源**。
XFF 为空时模块什么都不做，`$remote_addr` 保持最后一跳的原值。
判据看日志的 `xff=` 字段：
...
`xff="-"` 即表示链路上无人写 XFF。此时第 1 列与 `hop=` 必然相同 ——
这不是配置错误，而是**信息在网络层就丢了**。

各部署形态能否拿到真实 IP：

| 部署形态 | 第 1 列 | 原因 |
|---------|---------|------|
| 前面有 HTTP 代理 / 网关 / LB | ✅ 真实 IP | 上游写 XFF，realip 正常解析 |
| Docker `-p` 端口映射 | ⚠️ 网关 IP | 端口映射是 **L4 转发**，不添加任何 HTTP 头 |
| Podman rootless `slirp4netns` | ⚠️ `10.0.2.100` | **用户态网络栈**，真实来源在进入容器前即丢失 |
| `--network host` | ✅ 真实 IP | 容器直接用宿主机网络栈 |
| Podman `--network pasta` | ✅ 真实 IP | slirp4netns 的继任者，保留源 IP（Podman ≥ 4.4） |

**关于 slirp4netns**（Podman rootless 默认，最容易被忽略的坑）：
它是一套**完全在用户态模拟**的 TCP/IP 栈。外部连接到达宿主机端口后，
流量被 slirp 接管并**以虚拟网关身份重新向内转发**，内核并不知道真实来源。
故容器内无论怎么抓，看到的永远是 `10.0.2.100`（slirp 网的容器地址）。
这不是 Nginx 能解决的问题 —— **加任何 realip 配置都无效**，只能换网络模式：

```bash
podman run -d --network pasta ...     # 推荐：保留源 IP
podman run -d --network host ...      # 备选：注意 host 下 -p 无效
```

> 早期版本的本文档与 CHANGELOG 曾把「真实 IP 一直都在 XFF 里」写成普适结论，
> 那是**误读 `xff="-"`**（把空值的显示当成了有值）所致。XFF 只在上游主动
> 写入时才存在，端口映射与 slirp4netns 两种形态都不满足这个前提。此处已更正。
...
| `xff=` | `$http_x_forwarded_for` | 完整 XFF 链；`-` 表示无 XFF |
...
数据分析插件 `@chengxianglibra/dsh-data-analysis` 依赖本地 Python，
还要 pip 拉 `marivo`，装起来慢、体积大，但多数部署用不到。需要一个能关掉它的开关。

### `enabled` 字段：缺省即启用

`versions.yml` 里每个插件支持 `enabled`，语义是**缺省即启用**：
...
plugins:
  - name: some-plugin
    version: latest
    profile: web
    enabled: false      # 不写 或 true → 安装；false → 跳过
```

选「缺省启用」而不是「缺省关闭」是为了**向后兼容**：老配置文件里没有
`enabled` 字段，行为必须和以前完全一致，否则升级即破坏。

### 为什么用字符串比较，而不是 YAML 解析

本项目刻意不引入 YAML 库（不让脚本依赖 python/jq），用的是纯 bash 逐行正则解析。
这带来一个**反直觉的好处**：`enabled: false` 拿到的就是字符串 `"false"`。

如果经过 YAML 库，`false` 会被解析成布尔 `False`、`0` 会被解析成整数 `0`，
比较逻辑要处理多种类型；纯 bash 则统一按字符串处理，语义清晰。
判定函数接受 `true/1/yes/on`（大小写不敏感）为启用，其余一律跳过 ——
**未知值保守跳过**，避免拼错字段名导致意外安装大插件。
...
| 位置 | 时机 | 原来读什么 |
|------|------|-----------|
| `install-dsh.sh` | 首次安装（`dsh` 不存在时） | `versions.yml` |
| `entrypoint.sh` 的 `ensure_plugin` | 每次启动（补装兜底） | 硬编码插件名单 |

这是两套独立的判断。加开关时如果只改一边，就会出现：

- 只在 yml 里关掉 → 首装跳过，但重启时补装逻辑又把它装回来，**开关形同虚设**；
- 只在 entrypoint 关掉 → 首装仍然会装，**默认不装的目标没达成**。

所以 `entrypoint.sh` 也去读 `versions.yml`。但它**不能 source `install-dsh.sh`** ——
后者末尾无条件执行 `main`，source 会把整套安装流程重跑一遍。
故那里重新实现了一份极简 `awk` 解析（`plugin_enabled_in_versions`），
语义与 `parse_versions_file` 对齐：只在该插件自己的缩进段内找 `enabled`。

### 环境变量覆盖

`-e DSH_ENABLE_DATA_ANALYSIS=true` 可绕过配置直接启用，优先级最高：

```
环境变量 > versions.yml 的 enabled > 缺省(true)
```

两侧（安装与补装）都实现了同样的覆盖，否则会出现「首装认环境变量、
补装不认」的不一致 —— 这正是实测中发现并修掉的问题。

> 注意 `profile.env` 里**没有**给这个变量设默认值（保持未设置状态）。
> 若在那里写死 `export DSH_ENABLE_DATA_ANALYSIS="false"`，
> 它会永久覆盖 `versions.yml` 的设定，用户在 yml 里改成 `true` 也不生效 ——
> 又一个「双份事实来源」的坑。需要固定行为时再取消那行注释。

### 关掉插件时的用户预期管理

插件不装了，用户看到界面里功能消失会困惑。所以跳过时打印明确原因和开启方法：
...
| 场景 | yml `enabled` | 环境变量 | 结果 |
|------|--------------|---------|------|
| 默认 | `false` | 未设 | 不装，打印跳过提示 |
| 改配置 | `true` | 未设 | 正常安装 |
| 临时开 | `false` | `true` | 正常安装（环境变量覆盖） |
